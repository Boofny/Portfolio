# Portfolio TODO

- [ ] 1. get email js setup and working 
- [X] 2. get second project inside of main branch the neovim plugin and include some more like hardware embedded projects
- [ ] 3. keep rg TODO to see what i have to do
- [ ] 4. need to add a desc to the neovim plugin project

## golive card 
- [ ] adding a link to the live api 

## general todos 
- [ ] Make another page for more projects 
- [ ] restructure how skills is used to make it more moduler
- [ ] Add a contant be section
- [ ] maybe change the layout of skills and expeerience 

### some ideas
- Live API URL — proves it runs

